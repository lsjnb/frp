...
def get_github_latest_release():
    print(f"\n=== Starting GitHub sync at {time.strftime('%Y-%m-%d %H:%M:%S')} ===")

    g = Github()
    repo = g.get_repo("lsjnb/frp")
    release = repo.get_latest_release()
    
    if release:
        print(f"[INFO] Found release {release.tag_name} (published at {release.created_at})")
        print(f"[DEBUG] Release body: \n {release.body[:200]}...")
        files = []
        for asset in release.get_assets():
            ...
            if response.status_code == 200:
                ...
                print(f"[SUCCESS] Downloaded {name} ({file_size:.2f}MB) in {dl_time:.2f}s")
            else:
                print(f"[ERROR] Failed to download {name} (status: {response.status_code})")
...
def delete_gitee_releases(latest_id, client, uri, token):
    ...
    for id in release_ids:
        ...
        if delete_response.status_code == 204:
            ...
            raise ValueError(f"Request failed with status code {delete_response.status_code}")
...
def sync_to_gitee(tag: str, body: str, files: slice):
    ...
    if release_api_response.status_code == 201:
        ...
        print(f"Request failed with status code {release_api_response.status_code}")
    ...
    for file_path in files:
        ...
        while not success:
            ...
            if asset_api_response.status_code == 201:
                ...
                print(f"Request failed with status code {asset_api_response.status_code}")
